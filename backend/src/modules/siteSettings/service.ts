...
// FIXES:
//  - getAppLocalesMeta fallback list derives from env APP_LOCALES/SUPPORTED_LOCALES
// =============================================================
...
// preferred fallback kaldırıldı: default locale artık DB’den geliyor
...
type GetAppLocalesMetaOptions = {
  /** DB boşsa statik fallback listesine düş */
  allowFallback?: boolean;
};

const APP_LOCALES_FALLBACK: AppLocaleMeta[] = (() => {
  const raw = process.env.APP_LOCALES || process.env.SUPPORTED_LOCALES || '';
  const codes = uniqLocales(raw.split(/[;,]+/).map((x) => x.trim()));
...
export async function getAppLocalesMeta(
  opts: GetAppLocalesMetaOptions = {},
): Promise<AppLocaleMeta[]> {
  const allowFallback = opts.allowFallback !== false;
  const raw = await getGlobalSettingValue('app_locales');
  if (!raw) {
    return allowFallback ? APP_LOCALES_FALLBACK.slice() : [];
  }
...
export async function getDefaultLocale(_locale?: string | null): Promise<string | null> {
...
export async function getEffectiveDefaultLocale(): Promise<string> {
  const def = normalizeLocaleLoose(await getDefaultLocale(null));
  const metas = await getAppLocalesMeta({ allowFallback: false });
  const active = metas.filter((m) => m.is_active !== false);

  if (def && active.some((m) => m.code === def)) return def;

  const fromMeta = active.find((m) => m.is_default)?.code;
  if (fromMeta) return fromMeta;

  if (active[0]?.code) return active[0].code;
  if (def) return def;

  // Son çare: i18n runtime default (DB cache + env fallback)
 ...
 *  4) preferred fallback (opsiyonel, varsa)
 *  5) app_locales (aktif)
 *  6) '*' (GLOBAL)
 */
export async function buildLocaleFallbackChain(opts: {
  ...
  const preferred = normalizeLocaleLoose(opts.preferred) || null;
