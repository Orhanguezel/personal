...
import { extractHtmlFromPost, joinUrl, pickCover, sanitizeBlogHtml, toAbsUrl } from '@/integrations/shared';
...
export default async function BlogDetailPage({
  ...
  const fallbackCover = '/assets/imgs/blog/blog-1/img-background.webp';
  const absImg =
  ...
  const articleJsonLd = post
    ...
    : null;
  const initialHtml = sanitizeBlogHtml(extractHtmlFromPost(post as CustomPageView | null));
  ...
  return (
    <Layout headerStyle={1} footerStyle={1}>
      ...
      <BlogDetail locale={locale} slug={slug} initialPost={post} initialHtml={initialHtml} />
    </Layout>
  );
}
