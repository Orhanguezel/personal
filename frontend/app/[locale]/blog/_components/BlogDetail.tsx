...
type Props = {
  locale: string;
  slug: string;
  initialPost?: CustomPageView | null;
  initialHtml?: string;
};

export default function BlogDetail({ locale, slug, initialPost = null, initialHtml = '' }: Props) {
  const safeLocale = locale || 'en';
  const safeSlug = (slug || '').toString();
  const initialPostForSlug =
    initialPost && String(initialPost.slug ?? '') === safeSlug ? initialPost : null;
  const initialHtmlForSlug = initialPostForSlug ? initialHtml : '';
  ...
  const [post, setPost] = useState<CustomPageView | null>(initialPostForSlug);
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!safeSlug) return;
      if (initialPostForSlug) {
        setPost(initialPostForSlug);
  ...
  }, [safeSlug, safeLocale, triggerDetail, initialPostForSlug]);
  ...
  const allPosts = useMemo(
    () => (listLocale.data?.length ? listLocale.data : listFallback.data) || [],
    [listFallback.data, listLocale.data],
  );
  ...
  const html = useMemo(
    () => initialHtmlForSlug || sanitizeBlogHtml(rawHtml),
    [initialHtmlForSlug, rawHtml],
  );
                ...
                <div className="col-12 mt-6">
                  <div className="border-linear-3 rounded-4 p-lg-6 p-md-4 p-4">
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: sanitizeHtml(copy.author_bio_html) }}
                    />
                  </div>
                </div>
              ) : null}
        ...
        <div className="container border-top pt-80 pb-80">
          ...
          <div className="row mt-8">
            {(isLoadingList ? [] : related).map((p, idx) => {
              ...
              const imgFallback = `/assets/imgs/blog/blog-1/img-${(idx % 3) + 1}.webp`;
