...
class HoverImgFx1 {
    ...
    setupRevealContent() {
        ...
        hoverContent.className = 'tg-hover-wrapper';

        const author = this.DOM.el.dataset.author?.trim();
        const date = this.DOM.el.dataset.date?.trim();
        if (author || date) {
            const metaList = document.createElement('ul');
            metaList.className = 'tgbanner__content-meta list-wrap';
            ...
            if (author) {
                ...
                authorItem.append(label, document.createTextNode(' '), link);
                metaList.appendChild(authorItem);
            }
            ...
            hoverContent.appendChild(metaList);
        }

        const title = document.createElement('h3');
        title.className = 'tg-hover-title';
        title.textContent = this.DOM.el.dataset.title || '';
        hoverContent.appendChild(title);

        this.DOM.revealImg.appendChild(hoverContent);
