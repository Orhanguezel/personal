// =============================================================
// FILE: frontend/components/sections/services/Service2.tsx
// FINAL — Service2 (client, single file)
// - Stil/markup korunur
// =============================================================
...
import { safeParseServiceContent, normalizeUiServicesSettingValue, sanitizeHtml } from '@/integrations/shared';
...
/**
 * Bu section'da iconlar template'de statik SVG.
 * DB'de icon alanı olmadığı için deterministik bir icon seçiyoruz:
 * - index'e göre 7 icon döndürür (servis sayısı artarsa cycle eder)
 */
function pickIcon(idx: number) {
  const n = idx % 7;
...
export default function Service2({ locale }: { locale: string }) {
  ...
  const { data, isLoading, isFetching, isError } = useListServicesPublicQuery({
    locale,
    limit: 7, // bu section grid'i 7 kart tasarlanmış; seed 4 ise 4 basar
    offset: 0,
  ...
  const cards: CardVM[] = useMemo(() => {
    if (showBusy && items.length === 0) {
      return Array.from({ length: 6 }).map((_, idx) => ({
        id: `loading-${idx}`,
        href: '#',
        title: ui.section2.loading_title,
        text: ui.section2.loading_text,
      }));
    }
    ...
    return items.map((svc) => {
      ...
      return {
        id: svc.id,
        href,
        title: ((svc as any).name as string | null) ?? ui.section2.card_fallback_title,
        text,
      };
    });
            ...
            <div className="box-linear-animation position-relative z-1 p-lg-5 p-1 p-md-4">
              <div className="position-relative z-1">
                <div className="text-center">
                  ...
                  <h3 dangerouslySetInnerHTML={{ __html: sanitizeHtml(ui.section2.title_html) }} />

                  {isError ? (
                    <div className="mt-3">
                      <span className="fs-7 text-300">{ui.section2.error}</span>
                    </div>
                  ) : null}
                </div>
                ...
                <div className="container mt-5">
                  <div className="row g-4">
                    {cards.map((c, idx) => (
                      <div key={c.id} className="col-lg-4 col-md-6">
                        {/* Link'i kartın içine ekliyoruz ama markup bozulmasın diye wrapper yapmıyoruz */}
                        <div className="card-servies-2 rounded-2 h-100 hover-up">
                          {pickIcon(idx)}
                          <h6 className="my-3 fw-medium">
                            <Link href={c.href} className="text-reset">
                              {c.title}
                            </Link>
                          </h6>
                          <p className="fs-7 text-300 fw-regular">
                            {c.text}{' '}
                            <Link href={c.href} className="text-secondary-2">
                              {ui.section2.learn_more}
                            </Link>
                          </p>
                        </div>
                      </div>
                    ...
                    {!showBusy && !isError && cards.length === 0 ? (
                      <div className="col-12">
                        <div className="text-center">
                          <span className="fs-7 text-300">{ui.section2.empty}</span>
                        </div>
                      </div>
                    ) : null}
                  </div>
                  ...
                  <div className="text-center pt-60">
                    <p className="text-300">
                      <span dangerouslySetInnerHTML={{ __html: sanitizeHtml(ui.section2.footer_text_html) }} />{' '}
                      <Link href={`/${locale}/#contact`} className="text-primary-2">
                        {ui.section2.footer_link_label}
                      </Link>
                    </p>
                  </div>
                </div>
              ...
              <img
                ...
                src="/assets/imgs/home-page-2/services/bg.webp"
                alt="zelio"
              />
            </div>
          </div>
