...
/**
 * IMPORTANT:
 * - Seeder görselleri: "/assets/imgs/services-list/img-1.webp"
...
function normalizeHoverImgUrl(raw: string | null | undefined, fallbackIdx: number) {
  ...
  const fallback = `/assets/imgs/services-list/img-${(fallbackIdx % 4) + 1}.webp`;
  ...
  if (s0.startsWith('/assets/')) return s0.replace(
    /^\/assets\/imgs\/services-list\/img-([1-4])\.png$/i,
    '/assets/imgs/services-list/img-$1.webp',
  );
  if (s0.startsWith('assets/')) return `/${s0}`.replace(
    /^\/assets\/imgs\/services-list\/img-([1-4])\.png$/i,
    '/assets/imgs/services-list/img-$1.webp',
...
export default function Service1({ locale, initialData }: { locale: string; initialData?: ServiceDto[] }) {
  ...
  const items = useMemo(
    () => (initialData && initialData.length > 0 ? initialData : (data?.items ?? [])),
  ...
  const cards: CardVM[] = useMemo(() => {
    ...
    if (showBusy && items.length === 0) {
      return Array.from({ length: 4 }).map((_, idx) => ({
        ...
        img: normalizeHoverImgUrl(`/assets/imgs/services-list/img-${(idx % 4) + 1}.webp`, idx),
      }));
    }
    ...
    return items.slice(0, 4).map((svc, idx) => {
      ...
      // DB seed eski PNG değerleri normalizeHoverImgUrl ile WebP'ye taşınır.
          ...
          <div className="row align-items-end">
            <div className="col-lg-7 me-auto">
              ...
              <span
                ...
                dangerouslySetInnerHTML={{ __html: sanitizeHtml(ui.section1.intro_html) }}
              />
