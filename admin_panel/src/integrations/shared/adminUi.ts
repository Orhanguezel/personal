...
export function normalizeAdminUiCopy(raw: unknown): AdminUiCopy {
  ...
  const items: AdminNavCopy['items'] = {
    ...
    blog: withDefault(itemsRaw.blog, (ADMIN_UI_DEFAULTS.nav.items as Record<string, string>).blog),
    legal: withDefault(itemsRaw.legal, (ADMIN_UI_DEFAULTS.nav.items as Record<string, string>).legal),
    corporate: withDefault(itemsRaw.corporate, (ADMIN_UI_DEFAULTS.nav.items as Record<string, string>).corporate),
