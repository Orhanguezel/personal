...
function parseAppLocalesValue(raw: unknown): AppLocaleItem[] {
  ...
  if (Array.isArray(raw)) {
    return raw
      .map((item: unknown) => {
        ...
        return {
          code: toShortLocale(obj?.code ?? item),
          label: typeof obj?.label === 'string' ? obj.label : undefined,
          is_active: typeof obj?.is_active === 'boolean' ? obj.is_active : undefined,
          is_default: typeof obj?.is_default === 'boolean' ? obj.is_default : undefined,
        };
      })
      .filter((x) => !!x.code);
  }
  ...
  // DB object: { locales: [...] }
  if (isRecord(raw)) {
    if (Array.isArray(raw.locales)) return parseAppLocalesValue(raw.locales);
  }
...
export function useAdminLocales(): UseAdminLocalesResult {
  ...
  const computed = useMemo(() => {
    const list = (rows ?? []) as SiteSettingLike[];
    const appRow = list.find((r) => r.key === 'app_locales');
    const defRow = list.find((r) => r.key === 'default_locale');
