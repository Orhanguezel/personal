...
export default function AdminMenuItemClient() {
  ...
  const handleToggleActive = async (item: AdminMenuItemDto) => {
    try {
      ...
      toast.success(item.is_active ? 'Pasif edildi' : 'Aktif edildi');
    } catch (err) {
      toast.error(getErrMsg(err));
    }
  };
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <CardTitle className="text-2xl font-bold">Menü Öğeleri</CardTitle>
              <CardDescription>
                Tüm menü öğelerini yönetin ({displayItems.length} öğe)
              </CardDescription>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
              ...
              {/* Active Filter */}
              <div className="space-y-2">
                <Label htmlFor="activeFilter">Durum</Label>
                <Select
                ...
                {displayItems.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={reorderMode ? 8 : 7} className="h-24 text-center text-muted-foreground">
                      {busy ? 'Yükleniyor...' : 'Menü öğesi bulunamadı'}
                    </TableCell>
                  </TableRow>
                )}
                ...
                <CardContent className="p-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs">
                      ...
                      <span className="text-muted-foreground">Sıra: {item.display_order}</span>
                    </div>
                  </div>
                  {!reorderMode && (
